...
function StatusBadge({ status }: { status: string }) {
  const s = STATUS_STYLES[status] ?? { label: status, cls: "bg-sand-100 text-sand-600" }
  return (
...
export default async function OrdersPage() {
  ...
  return (
    <main>
      <h1 className="text-3xl font-bold text-sand-900 mb-8">Order History</h1>
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center rounded-xl border border-dashed border-sand-300 bg-white">
          ...
          <h2 className="text-lg font-semibold text-sand-700">No orders yet</h2>
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <Link
              ...
              className="block rounded-xl border border-sand-200 bg-white p-5 hover:border-brand-300 hover:shadow-sm transition-all"
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right flex-shrink-0">
                  <p className="font-semibold text-sand-900">{formatAmount(order.total)}</p>
